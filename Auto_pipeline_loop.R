...
for (i in fcs_names) {  
  ...
  legend("topleft",legend = c(paste0("count: ",cd45@cell.count),
                              paste0("frequency: ",round(cd45@proportion,digits = 2))
                              ,paste0('total: ',sngl@cell.count)),bty = "n",text.col=1)
  dev.off()
  ...
  legend("topleft",legend = c(paste0("count: ",live@cell.count),
                              paste0("frequency: ",round(live@proportion,digits = 2))
                              ,paste0('total: ',cd45@cell.count)),bty = "n",text.col=1)
  ...
  legend("topleft",legend = c(paste0("count: ",cd3@cell.count),
                              paste0("frequency: ",round(cd3@proportion,digits = 2))
                              ,paste0('total: ',live@cell.count)),bty = "n",text.col=1)
  dev.off()
  ### CD8 CD4
  ...
  #not_cd45 <- flowDensity(after, channels=c("AmCyan-A", "Qdot 655-A"), position=c(F,NA),upper=c(F,NA))
  bmp(file="CD8_CD4.bmp",
      width=6, height=4, units="in", res=100)
  plotDens(after,c("DAPI-A","APC-A"),main='CD8/CD4')
  lines(cd8@filter,type="l",col=2)
  lines(cd4@filter,type="l",col=3)
  legend("topleft",legend = c(paste0("count: ",cd8@cell.count),
                              paste0("frequency: ",round(cd8@proportion,digits = 2))
                              ,paste0('total: ',cd3@cell.count)),bty = "n",text.col=2)
  legend("bottomright",legend = c(paste0("count: ",cd4@cell.count),
                                  paste0("frequency: ",round(cd4@proportion,digits = 2))
                                  ,paste0('total: ',cd3@cell.count)),bty = "n",text.col=3)
  dev.off()
  ### tregs
  ...
  legend("topleft",legend = c(paste0("count: ",tregs@cell.count),
                              paste0("frequency: ",round(tregs@proportion,digits = 2))
                              ,paste0('total: ',cd4@cell.count)),bty = "n",text.col=1)
  dev.off()
  ### Neutrophils
  ...
  legend("topleft",legend = c(paste0("count: ",neutro@cell.count),
                              paste0("frequency: ",round(neutro@proportion,digits = 2))
                              ,paste0('total: ',not_cd3@cell.count)),bty = "n",text.col=1)
  dev.off()
  ### Macrophages
  ...
  legend("topleft",legend = c(paste0("count: ",cd206@cell.count),
                              paste0("frequency: ",round(cd206@proportion,digits = 2))
                              ,paste0('total: ',macro@cell.count)),bty = "n",text.col=1)
  dev.off()
  ### Eosinophils
  ...
  bmp(file="Monocytes.bmp",
      width=6, height=4, units="in", res=100)
  plotDens(not_nk,c("DAPI-A","APC-A"),main='Monocytes')
  #lines(not_cd45@filter,type="l",col=2,lwd=4)
  lines(nk@filter,type="l")
  #lines(not_neutro@filter,type="l")
  legend("topleft",legend = c(paste0("count: ",mono@cell.count),
                              paste0("frequency: ",round(mono@proportion,digits = 2))
                              ,paste0('total: ',not_nk@cell.count)),bty = "n",text.col=1)
  dev.off()
  ### creating dataframe with info on percentages and counts
  Cell <- c('Singlets','CD45','Live','CD3+','CD3-','CD8 T cells','CD4 T cells','Tregs','Neutrophils','Not Neutrophils','Marophages',
            'Not Macrophages','CD206 high macrophages','CD206 low macrophages','Eosinophils','NK cells','Not NK','Monocytes')
  Cell_count <- c(sngl@cell.count,cd45@cell.count,live@cell.count,cd3@cell.count,not_cd3@cell.count,
  ...
  Cell_parent_percentage <-c(sngl@proportion,cd45@proportion,live@proportion,cd3@proportion,not_cd3@proportion,
